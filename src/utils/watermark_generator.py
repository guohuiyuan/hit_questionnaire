...
def compress_image(image, output_path, quality=85, optimize=True):
    ...
    try:
        if output_path.lower().endswith(('.jpg', '.jpeg')):
            ...
            image.save(output_path, compress_level=6)  # PNG压缩级别 (0-9)
        else:
            image.save(output_path)
    except Exception as e:
        print(f"压缩图片时出错: {output_path} - {e}")

def add_watermark(image_path, output_path, watermark_text, opacity=30, scale=0.8, angle=30, 
                  position='tiled', color='gray', compress=False, quality=85):
    """
    添加水印并可选压缩图片
    """
    try:
...
def process_directory(input_dir, output_dir, watermark_text, compress=False, quality=85, **kwargs):
    """
    处理目录中的所有图片
    """
...
def main():
    ...
    parser.add_argument('-a', '--alpha', type=int, default=30, help='水印透明度 (0-100), 默认: 30')
    parser.add_argument('-s', '--scale', type=float, default=0.8, help='水印缩放比例, 默认: 0.8')
    parser.add_argument('-r', '--rotate', type=int, default=30, help='水印旋转角度, 默认: 30度')
    parser.add_argument('-p', '--position', choices=['tiled', 'center', 'random'], default='tiled', 
